-- ChordCraft Database Schema for Supabase
...
-- Create custom types
CREATE TYPE subscription_status AS ENUM ('active', 'canceled', 'incomplete', 'incomplete_expired', 'past_due', 'unpaid', 'trialing');
...
CREATE TABLE IF NOT EXISTS public.project_tracks (
    ...
    name TEXT NOT NULL,
    type TEXT NOT NULL DEFAULT 'synthesizer',
...
CREATE TABLE IF NOT EXISTS public.usage_tracking (
    id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
    user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
    subscription_id UUID REFERENCES public.stripe_subscriptions(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_profiles_user_id ON public.user_profiles(id);
CREATE INDEX IF NOT EXISTS idx_profiles_stripe_customer ON public.user_profiles(stripe_customer_id);
...
CREATE POLICY "Users can view their own payments" ON public.stripe_payments FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can create their own payments" ON public.stripe_payments FOR INSERT WITH CHECK (auth.uid() = user_id);

-- Stripe subscriptions policies
CREATE POLICY "Users can view their own subscriptions" ON public.stripe_subscriptions FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can create their own subscriptions" ON public.stripe_subscriptions FOR INSERT WITH CHECK (auth.uid() = user_id);
...
BEGIN
    ...
    LEFT JOIN public.stripe_subscriptions ss ON up.subscription_id = ss.stripe_subscription_id
    WHERE up.id = user_uuid;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Create a function to check if user has access to feature
...
BEGIN
    SELECT up.subscription_plan, up.subscription_status
    ...
    CASE feature_name
        WHEN 'unlimited_projects' THEN
            RETURN user_plan IN ('pro', 'studio', 'enterprise');
        WHEN 'advanced_ai' THEN
            RETURN user_plan IN ('pro', 'studio', 'enterprise');
        WHEN 'collaboration' THEN
            RETURN user_plan IN ('studio', 'enterprise');
